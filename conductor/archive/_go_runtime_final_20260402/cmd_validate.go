...
func runValidate() {
	...
	// Validate tracks
	tracksFile := filepath.Join(*projectDir, "conductor", "tracks.md")
	...
	// Validate tasks
	var fsTaskCount int
	conductorTracksDir := filepath.Join(*projectDir, "conductor", "tracks")
	if entries, err := os.ReadDir(conductorTracksDir); err == nil {
		for _, entry := range entries {
			if !entry.IsDir() {
				continue
			}
			planPath := filepath.Join(conductorTracksDir, entry.Name(), "plan.md")
	...
	for _, dir := range []string{"open", "resolved"} {
		issueDir := filepath.Join(*projectDir, "conductor", "broker", dir)
	...
	// Validate logs
	var fsLogLines int
	logsDir := filepath.Join(*projectDir, "conductor", "logs", projectID)
