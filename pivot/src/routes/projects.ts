...
import {
  registerProjectRunRoute,
  type ProjectRunDependencies,
} from './projectRun'

export { makeTrackId, extractGoalFromSpec, mergeStoriesSection } from '../sync/storyGenerator'
export type { ProjectRunDependencies } from './projectRun'
 ...
 * @param projectRunDependencies - Lower-level executor/preflight seams for integration tests.
 */
export function registerProjectRoutes(
  router: Router,
  client: ConvexHttpClient,
  storyRunner?: StoryGenerationRunner,
  projectRunDependencies?: ProjectRunDependencies,
): void {
  ...
  registerProjectRunRoute(router, client, projectRunDependencies)
  ...
  router.post('/api/projects/:id/tracks/:trackId/generate/commit', async (request, params) => {
    ...
    return json(
      { projectSlug: project.slug, trackId, stories: stories.length, tasks: createdTasks },
      201,
    )
  })
