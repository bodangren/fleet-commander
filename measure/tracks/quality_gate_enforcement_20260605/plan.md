...
- [~] Task: Triage the current orphan list into wire / delete decisions; record in tech-debt. (Documentation/manual; not driven by a Red test — the test-strategy does not include a triage test. Out of scope for the Red phase.)
  - **Red-phase review (2026-06-06, mid agent):** all 18 Red-phase tests in `measure/tests/orphans.test.sh` pass (`bash measure/tests/orphans.test.sh` → 18/18). Phase 3 Red is complete; this triage task remains open and is the only pending Phase 3 work. Marked `[~]` to evidence the Red-phase review is in progress; owned by a documentation/manual role, not the Red-phase mid agent. Inputs the triage role needs: (1) live `bash measure/doctor.sh orphans` output, (2) current `measure/orphans-allowlist.txt`, (3) `measure/tech-debt.md` (TD-209, TD-213, TD-238).
