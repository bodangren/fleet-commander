...
import (
	...
	"github.com/measure/fleet-commander/internal/watcher"
)
...
func main() {
	...
	// Load persisted projects from ~/.measure/projects.json
	...
	if err != nil {
		...
		if err != nil {
			...
			for _, entry := range entries {
				...
				projectLoggers[p.ID] = logs.NewLogger(filepath.Join(entry.Path, "measure", "logs"), p.ID)
	...
	if err != nil {
		log.Printf("Warning: Failed to watch measure directory: %v", err)
	}
	...
	logsDir := filepath.Join(currentDir, "measure", "logs")
