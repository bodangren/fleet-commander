...
import (
	...
	"github.com/conductor/fleet-commander/internal/storage"
)

const BundledDir = "defaults/harnesses"
...
func (s *Store) ResetUser(name string) error {
	...
	for _, ext := range []string{".yaml", ".yml"} {
		path := filepath.Join(s.userDir, fmt.Sprintf("%s%s", sanitizeName(name), ext))
...
func (s *Store) ResetProject(name string) error {
	...
	for _, ext := range []string{".yaml", ".yml"} {
		path := filepath.Join(s.projectDir, fmt.Sprintf("%s%s", sanitizeName(name), ext))
		if err := os.Remove(path); err != nil && !os.IsNotExist(err) {
			return err
		}
	}
...
func readDefinitionsFromFS(source fs.FS, dir string, layer Layer) (map[string]ResolvedDefinition, error) {
	...
	if err != nil {
		...
		return nil, fmt.Errorf("failed to read bundled harnesses: %w", err)
	}
	...
	for _, entry := range entries {
		...
		if err != nil {
			return nil, fmt.Errorf("failed to read bundled harness: %w", err)
		}
		...
		key := def.Name
		result[key] = ResolvedDefinition{Definition: def, Layer: layer}
	}
	return result, nil
}
...
func readDefinitionsFromDir(dir string, layer Layer) (map[string]ResolvedDefinition, error) {
	...
	if err != nil {
		...
		return nil, fmt.Errorf("failed to read harness dir: %w", err)
	}
...
func saveDefinition(dir string, def *Definition) error {
	...
	filename := fmt.Sprintf("%s.yaml", sanitizeName(def.Name))
