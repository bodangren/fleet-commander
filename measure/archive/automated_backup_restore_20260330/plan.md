...
- [x] Task: Implement project-level backup
  - Locate project measure directory from `projects.json` registry
  - Zip the measure directory and write to `~/.measure/backups/` with timestamped filename
  - Write test: `CreateProjectBackup("myapp")` produces zip containing expected files
- [x] Task: Implement full system backup
  - Zip entire `~/.measure/` directory excluding `backups/` to avoid recursion
...
- [x] Task: Add `measure backup create` CLI command
  - Flags: `--project <name>`, `--full`, `--output <path>`
  - Print backup file path and size on success
- [x] Task: Add `measure backup restore` CLI command
...
- [x] Task: Update `measure/tracks/automated_backup_restore_20260330/plan.md` to mark phases complete
