...
| TD-266 | Fleet bootstrap readiness: shared `useFleetData` waits on health, projects, agents, and harnesses together, so slow optional reads delay project selection. Decouple truthful project readiness from optional agents/harnesses without read-side mutation or API expansion unless characterization proves it necessary; local evidence includes `/api/projects` up to 13.1s and a 1.775s harness read. | High | `fleet_bootstrap_readiness_recovery_20260809` |
