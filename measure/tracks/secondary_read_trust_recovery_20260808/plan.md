...
- `measure/doctor.sh all` passes the as-any, boundary, stub-mutation, and status-vocabulary guards, then exits 1 on the pre-existing 516-line `qualityWorkflowRunner.ts`, 38 stale orphan-allowlist entries, and 56 reported orphan exports. Several reported “orphans” are demonstrably production-wired through barrels/routes in this track (`useAuditEventsState`, `useConvexQueryState`, `useReconciliationProposalsState`, `useSelectedProject`, and `NotFoundPage`), so doctor output is retained as graph/tooling debt rather than used for unsafe deletion. This repository has no `measure/generate.sh`.
