...
- **FR5**: Backend persists settings to `~/.measure/config.json` via a new `ConfigManager` type in `internal/config/`, supporting `Get`, `Set`, and `Load` operations.
...
3. `PUT /api/settings` accepts partial updates and persists them to `~/.measure/config.json`.
