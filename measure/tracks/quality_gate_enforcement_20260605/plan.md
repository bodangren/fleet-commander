...
- [~] Task: Add to `measure/workflow.md` a closeout gate: "a track may be archived only when `verify` passes and the orphans report is clean (or new orphans are allowlisted with a TD id)." (Red-phase tests in `measure/tests/closeout.test.sh` pin the rule phrase, structure, and keyword requirements; currently fail because the rule is absent from `workflow.md`.)
- [~] Task: Run `verify`; record an all-green result (coordinating with the tracks that own the current red tests). (Red-phase tests assert `verify.sh` can produce a structured all-greens run; the all-green result is recorded once the upstream red gates are resolved.)
- [~] Task: Update `build-graph`; commit and push. (Red-phase test asserts `graph.db` is fresh; currently passes — included as a regression guard for the closeout gate.)
