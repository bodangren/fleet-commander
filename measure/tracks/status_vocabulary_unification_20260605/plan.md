...
- [~] Task: Add a doctor check (or extend Check 4 family) that flags new inline status `v.union(v.literal(...))` in `convex/schema/**` not sourced from `validators.ts`; allowlist current exceptions; negative-test it. _(Red-phase contract test in `measure/doctor/checks/status_vocabulary.test.ts`; planted fixture at `tests/fixtures/bad_schema.ts`; will fail until Green phase implements the check. State of `convex/schema/**` confirmed clean today: `rg 'v\.union\(v\.literal' convex/schema/` returns 0 matches — see inventory.md §1 for the consolidated validator set.)_
- [~] Task: Run all suites + `bun --cwd pivot typecheck` + `doctor.sh all`; full green. _(Blocked on Task 1 Green — `doctor.sh all` cannot pass until the new check is registered.)_
- [~] Task: Update `build-graph`; add a `schema_status_drift` reinforcement note to lessons-learned if warranted. _(Reinforcement note lands alongside Task 1 Green — the test file already cites `lessons-learned.md::schema_status_drift` in its header, so a brief addition making the Phase 4 doctor check an enforcement mechanism is justified.)_
- [~] Task: Commit and push. _(Split into the Red-phase commit (this commit) and the Green-phase commit (after Tasks 1–3 land) so the Red-phase boundary remains test-only per the user instruction "do not implement feature logic".)_
