...
This is not a narrow storage migration. It is a controlled product rewrite that replaces the current runtime assumptions described in `conductor/product.md` and `conductor/tech-stack.md`.
...
- filesystem parsing and watching of `conductor/` artifacts
...
7. Documentation and directives in `conductor/` no longer present the Go daemon + SQLite architecture as the current target state after the pivot baseline is established.
