...
## Overview
The "spine" of the new architecture relies on watching the filesystem for changes across multiple registered projects and parsing Measure markdown files (`tracks.md`, `plan.md`) into structured data.

## Goals
- Implement a global project registry to keep track of which directories the daemon is managing.
- Integrate `fsnotify` to watch the `measure/` directory in each registered project.
...
- When `measure/tracks.md` or a `plan.md` file is manually edited, the daemon logs the parsed, updated state to the terminal.
