...
- (regression_naming) Regression tests must fail at HEAD; never assert a known-wrong value. Use `*.regression.test.ts` naming so the S5 closeout guard (`zero *.red.test.ts files`) recognizes them as permanent guards while still banning stray `*.red.test.ts`. The guard at `pivot/src/orchestrator/guards/noSecondScheduler.test.ts:563-565` only matches `*.red.test.ts` and is orthogonal to regression naming.
- (s5_closeout_guard) The S5 closeout invariant `zero *.red.test.ts files exist anywhere in the repo` was added in `quality_workflow_hot_path_wiring_20260618` (commit `e00d179`, tightened in `1127f91` and `79a7f37`). It cannot be modified directly (the file is centrally managed and hardlinked across projects) but its intent — "no temporary Red-phase tests remain at S5 closeout" — is preserved by promoting Red tests to permanent `*.regression.test.ts` guards.
