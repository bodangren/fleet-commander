...
- [~] Task: Verify `bun --cwd pivot typecheck` is 0 errors for all provider files; re-run the full suite. **Red phase: `bun --cwd pivot typecheck`** reports 2 TD-235 errors at `convex/providers.ts:199,213` (the two source-code targets called out by the plan) plus 2 pre-existing TD-237 errors at `convex/lib/insights.ts:77` and `convex/projects.ts:150` (out of scope for this track). Zero new errors. **Green phase: `71a7f8b`** — TD-235 errors resolved (0 new errors). 2 pre-existing TD-237 errors remain. `bun --cwd pivot test`: 1124 pass / 0 fail. `bun --cwd frontend test`: 875 pass / 4 fail (pre-existing DashboardPage).
  ...
  - **Mid Red-phase 2026-06-07 (this attempt):** confirmed the 4 committed-red failures via `bun test ./convex/providerHealth.test.ts` (26 pass / 4 fail). Failures: line 128 (`records a successful probe with low latency as healthy` — expected `'healthy'`, received `'active'`), line 234 (`transitions to unhealthy when failureCount >= 3…` — expected `'unhealthy'`, received `'active'`), line 269 (`transitions to degraded when avgLatencyMs > 10_000` — expected `'degraded'`, received `'active'`), line 380 (`getProviderHealth > includes health fields populated by updateProviderHealth` — expected `'healthy'`, received `'active'`). All 4 fail for the expected missing behavior: the source writes the new vocabulary to `healthStatus` (per `71a7f8b`), but the test assertions still read the old `status` field. Added new Red-phase regression tests in `convex/providerHealth.test.ts` that pin the new contract (BOTH `healthStatus` carries the health value AND operational `status` is preserved). These pass in the current implementation and protect against a regression to the old vocabulary.
