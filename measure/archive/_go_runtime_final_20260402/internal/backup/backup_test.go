...
func TestCreateProjectBackup(t *testing.T) {
	...
	if err := os.MkdirAll(filepath.Join(measureDir, "tracks"), 0755); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(measureDir, "tracks.md"), []byte("# Tracks"), 0644); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(measureDir, "tracks", "plan.md"), []byte("# Plan"), 0644); err != nil {
		t.Fatal(err)
	}
	...
	// Verify zip contains the measure files
...
func TestCreateFullBackup(t *testing.T) {
	...
	// Simulate ~/.measure structure
	if err := os.WriteFile(filepath.Join(measureDir, "config.json"), []byte(`{}`), 0644); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(measureDir, "projects.json"), []byte(`[]`), 0644); err != nil {
		t.Fatal(err)
	}

	// Create a backups/ dir that should be excluded
	backupsDir := filepath.Join(measureDir, "backups")
	...
	destDir := t.TempDir()
	zipPath, err := CreateFullBackup(measureDir, destDir)
	if err != nil {
