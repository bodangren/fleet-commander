    ...
    <Card className="border-border/60 bg-background/60">
      <CardHeader className="space-y-2">
        <div className="flex items-start justify-between gap-3">
          <div>
            <CardTitle className="text-base">{agent.definition.name}</CardTitle>
            <CardDescription>{agent.definition.description}</CardDescription>
          </div>
          <span className="rounded-full border border-border/60 px-2 py-1 text-xs uppercase tracking-[0.2em] text-muted-foreground">
            {agent.layer}
          </span>
        </div>
      </CardHeader>
      <CardContent className="space-y-2 text-sm">
        <Row label="Mode" value={agent.definition.mode} />
        <Row label="Model" value={agent.definition.model} />
