...
func (m *Migrator) MigrateProject(projectPath string) (*MigrationResult, error) {
	...
	if _, err := os.Stat(conductorDir); os.IsNotExist(err) {
		return nil, fmt.Errorf("no conductor directory found at %s", projectPath)
	}
	...
	tracksFile := filepath.Join(conductorDir, "tracks.md")
	...
	m.importIssuesFromDir(projectID, filepath.Join(conductorDir, "broker", "open"), "open", result)
	m.importIssuesFromDir(projectID, filepath.Join(conductorDir, "broker", "resolved"), "resolved", result)

	// Import execution logs from logs/ directory
	m.importLogs(projectID, filepath.Join(conductorDir, "logs", projectID), result)
