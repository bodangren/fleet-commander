...
- **FR5:** A Bun loader reads all `conductor/harnesses/*.yaml` and upserts profiles into Convex on startup and on file change.
...
2. `conductor/harnesses/opencode.yaml` updated with real values for both blocks; documented in a comment header.
...
- **Config:** YAML in `conductor/harnesses/`
