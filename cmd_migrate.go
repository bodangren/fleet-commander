...
func runMigrate() {
	...
	// Check if the root directory itself is a conductor project
	conductorDir := filepath.Join(*rootDir, "conductor")
	...
	// Otherwise scan for sub-directories with conductor/
	...
	for _, entry := range entries {
		...
		if _, err := os.Stat(filepath.Join(projectPath, "conductor")); os.IsNotExist(err) {
			continue
		}
