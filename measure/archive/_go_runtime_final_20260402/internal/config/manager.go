...
// ConfigManager handles reading/writing the global measure configuration.
type ConfigManager struct {
	configDir  string
	configFile string
}

// NewConfigManager creates a ConfigManager using ~/.measure as the config dir.
func NewConfigManager() (*ConfigManager, error) {
	homeDir, err := os.UserHomeDir()
	if err != nil {
		return nil, fmt.Errorf("failed to get home directory: %w", err)
	}
	configDir := filepath.Join(homeDir, ".measure")
