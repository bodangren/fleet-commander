...
### Test
- [~] Task: Write validator and mutation contract tests for valid profiles, invalid stage orders, duplicate stages, unsafe configuration, authorization, and version updates. _(Red evidence — `pivot/src/shared/qualityProfile.red.test.ts` + `convex/qualityProfiles.red.test.ts`; module under test does not exist yet, so both files fail at import resolution. Targeted Red command: `bun --cwd pivot test src/shared/qualityProfile.red.test.ts` + `bun test ./convex/qualityProfiles.red.test.ts` — both exit 1 with `Cannot find module './qualityProfile'` / `./qualityProfiles`. Pivot test file: 1 file, 1 error, 0 pass. Convex test file: 1 file, 1 error, 0 pass.)_
- [~] Task: Add backward-compatibility tests proving projects without a selected profile retain current orchestration behavior. _(Red evidence — same Red files. Pivot test file includes the no-selection resolver test (`resolveEffectiveProfile({})` returns the `none` built-in with empty stages and `source: 'default'`) and the "built-in none profile has no stages" identity test. Convex test file includes `getEffectiveProjectProfileHandler` resolving an unselected project to `none` with `profileVersion: 0`.)_
- [~] Task: Add snapshot tests proving a claimed run keeps its original profile version after the source profile changes. _(Red evidence — same Red files. Pivot test file includes `serializeQualityProfile` / `parseQualityProfileSnapshot` round-trip + tamper-detection tests, `isImmutableSnapshot` type-guard tests, and the resolver snapshot-mutation-leak test. Convex test file includes the `recordClaimedRunProfileHandler` "v1 snapshot stays v1 after v2 publish" test, plus the duplicate-runId idempotency-rejection test.)_
