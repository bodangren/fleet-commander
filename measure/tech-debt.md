...
| TD-232 | Project Template Marketplace Phase 3 task "Add Templates link to main navigation" marked [x] but sidebar label in `frontend/src/layout/AppLayout.tsx:67` is `"Templates"` instead of `"Project Templates"` required by Phase 3 Red-phase tests (`frontend/src/layout/AppLayout.test.tsx`). Surfaced by Phase 4 regression run: 5 AppLayout tests failing, asserting `getByRole('link', { name: /project templates/i })`. Phase 3 was committed in `b0743a3` despite failing contract. | High |
