...
- **FR1:** Support per-project static analysis tool configuration in `conductor/analysis.yml` with tool name, command, output format, and enabled flag.
...
1. `conductor/analysis.yml` defines tools with `name`, `command`, `output_format` (json/text), `severity_map`, and `enabled` fields.
...
8. Projects without `conductor/analysis.yml` skip analysis without error.
