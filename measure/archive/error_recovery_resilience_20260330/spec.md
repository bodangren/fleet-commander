...
1. A project with a corrupted `plan.md` (e.g., malformed YAML frontmatter) is listed as `degraded` in `measure status` and dashboard, with an error message citing the parse failure.
