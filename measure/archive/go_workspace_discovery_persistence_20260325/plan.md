...
- [x] Task: Implement logic to read/write a `projects.json` file in a global configuration directory (e.g., `~/.measure/projects.json`).
...
- [x] Task: Implement a recursive directory scanner `FindMeasureProjects(rootDir string) []string` that searches for subdirectories containing a `measure/` folder. Limit recursion depth for performance.
...
- [x] Task: Ensure the `WatcherService` is automatically hooked up to start monitoring the new `measure/` directories upon registration.
