...
- **FR1:** Add a migration command (`measure migrate`) that reads all existing filesystem state (projects, tracks, tasks, issues, logs) and inserts it into SQLite.
...
1. Running `measure migrate` imports all existing projects, tracks, tasks, issues, and execution logs into SQLite without data loss.
...
5. Running `measure validate` produces a report showing zero discrepancies on a freshly migrated database.
