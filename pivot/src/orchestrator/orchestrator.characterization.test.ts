...
function successfulExecute(taskKey: string, output = 'success', sessionId?: string): ExecuteFn {
  return mock(async () => ({
    taskKey,
...
function findMutation(client: RecordingClient, predicate: (args: Args) => boolean): Call[] {
...
describe('runProject characterization: happy path (ready -> merged)', () => {
  it('executes a single task, transitions to in_progress then done, and persists succeeded run', async () => {
    ...
    const result = await runProject(client as any, 'demo', FAST_RETRY_CONFIG, undefined, execute)
...
describe('runProject characterization: execution failure with recovery', () => {
  it('on persistent failure: marks task blocked, persists failed run, records circuit failure, and creates blocker', async () => {
    ...
    const result = await runProject(client as any, 'demo', RETRY_CONFIG, hooks, execute)
    ...
    const finalTaskStatus = findMutation(client, a => a.taskKey === 't1' && a.status === 'blocked')
    ...
    const retryEvents = findMutation(client, a => a.eventType === 'retry')
    const blockedEvents = findMutation(client, a => a.eventType === 'blocked')
  ...
  it('succeeds after one transient failure (retry-then-succeed path)', async () => {
    ...
    const result = await runProject(client as any, 'demo', RETRY_CONFIG, undefined, execute)

    expect(result.status).toBe('succeeded')
    expect(execute).toHaveBeenCalledTimes(2)

    const finalTaskStatus = findMutation(client, a => a.taskKey === 't1' && a.status === 'done')
    expect(finalTaskStatus.length).toBe(1)
...
describe('runProject characterization: quality workflow integration', () => {
  it('runs selected quality stages after atomic claim and before final success handling', async () => {
    ...
    expect(qualityHooks.recordProfileSnapshot).toHaveBeenCalledWith(
      client,
      'demo',
      't1',
      expect.any(String),
    )
    expect(qualityHooks.runner!.runStage).toHaveBeenCalledTimes(
  ...
  it('blocks downstream success handling when a required selected quality stage fails', async () => {
    ...
    const blockedMutations = findMutation(client, a => a.taskKey === 't1' && a.status === 'blocked')
