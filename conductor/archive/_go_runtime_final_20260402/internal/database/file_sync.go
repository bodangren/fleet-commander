...
func (s *FileSyncer) SyncIssuesToFile(projectPath string, issue *Issue) error {
	...
	if issue.Status == "resolved" {
		issueDir = filepath.Join(projectPath, "conductor", "broker", "resolved")
	}
...
func findPlanPathForTrack(projectPath, trackID string) string {
	conductorDir := filepath.Join(projectPath, "conductor", "tracks")
	entries, err := os.ReadDir(conductorDir)
	if err != nil {
		return ""
	}
	...
	for _, entry := range entries {
		...
		planPath := filepath.Join(conductorDir, entry.Name(), "plan.md")
...
func (l *FileLoader) LoadProjectsFromFiles(rootDir string) error {
	...
	for _, entry := range entries {
		...
		conductorPath := filepath.Join(projectPath, "conductor")

		if _, err := os.Stat(conductorPath); os.IsNotExist(err) {
			continue
		}
