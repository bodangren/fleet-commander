...
- (conditional_required_stages) A quality stage with conditional applicability (e.g. `trackIsSetup`) must be `required: false`. If `required: true` and the condition is false, the run fails immediately — which is correct per spec but breaks profiles that have conditionally-applicable required stages. The fix: make conditionally-applicable stages optional so they're skipped (not failed) when not applicable.
