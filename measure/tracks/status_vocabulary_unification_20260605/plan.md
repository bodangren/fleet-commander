...
- [x] Task: Where a UI renders a status, extract a single `{value: {label,color}}` map co-located with the type; update components to import it (removes drift between color/label maps). **REOPENED 2026-06-07 (review):** marked done in `fedbbc5` but two components still defined local `statusColors` maps — `frontend/src/components/providers/ProviderCard.tsx:18` and `frontend/src/components/kanban/DependencyEditor.tsx:20`. **Resolved in `fdf9df9`** by replacing both local maps with canonical display-map aliases; `bun test ./convex/lib/validators.test.ts` passes 503/503, including the three legacy-map contract tests for `DependencyEditor` and `ProviderCard`.
...
- [x] Task: Run all suites + `bun --cwd pivot typecheck` + `doctor.sh all`; full green. (19a9c1b — Phase 3 contract tests: 43/43 pass; Phase 4 contract tests: 9/9 pass; pivot typecheck: clean; `doctor.sh status-vocabulary`: PASS; `doctor.sh all` fails only on pre-existing Check 1 `as any` violations unrelated to this track. **Review fix `d59969c`**: added root `lint`/`build` scripts and fixed pre-existing lint/build errors in `ModelRouterSettings.tsx`, `ProjectViewPage.tsx`, `DependencyEditor.tsx`, `fleetTypes.ts`, `types.ts` so gate runner passes. **Review fix `c077936`**: added `ModelRouterSettings` to orphans allowlist (TD-240 false positive).)
