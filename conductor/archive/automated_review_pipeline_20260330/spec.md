...
- **FR1:** After task execution completes in the orchestrator, automatically run linter, type-checker, and test suite commands based on the project tech stack detected from `conductor/review.yml`.
...
- **FR6:** Support configurable review commands per project stored in `conductor/review.yml` with fields for linter, typecheck, and test commands plus optional timeout.

## Acceptance Criteria

1. Completing a task triggers review commands defined in `conductor/review.yml` without manual intervention.
...
6. Projects without `conductor/review.yml` skip the review step gracefully with a logged notice.
