...
We need to persist the list of registered projects to disk and provide an API that allows the frontend to request a scan of a generic "Workspace Root" (like `~/Desktop`) to find all projects that have a `conductor/` directory setup.

## 3. Architecture & Data Flow
1. **Config Manager:** A new package `internal/config` will handle reading/writing a `~/.conductor/projects.json` file.
