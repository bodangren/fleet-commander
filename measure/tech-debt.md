...
| TD-118 | Error boundary tests fail across Phase 7 hooks: React error propagation architecture doesn't surface thrown errors to `result.error` in vitest — deferred, requires reworking test approach | High     |
| TD-119 | Chart component tests fail in jsdom due to recharts ResponsiveContainer producing 0×0 SVG — fundamental jsdom/recharts incompatibility                                                      | Critical |
| TD-120 | ChartEdgeCases tests fail: same root cause as TD-119                                                                                                                                        | Critical |
