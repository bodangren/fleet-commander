...
- [x] Task: Initialize SQLite database on server startup in `main.go`
  - Open `~/.conductor/fleet_commander.db` using `modernc.org/sqlite` driver
...
- [x] Task: Implement `conductor migrate` CLI command
...
- [x] Task: Implement `conductor validate` command
...
- [x] Task: Update `conductor/tracks/sqlite_database_migration_20260330/plan.md` to mark phases complete
