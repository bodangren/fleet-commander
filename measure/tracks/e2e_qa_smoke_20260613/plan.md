...
### Implement
- [x] Task: `runNavigation(scenarios)` — for each scenario: **GREEN landed 2026-06-13** (`9243185`), **GREEN fix 2026-06-13** (`b23010a`).
  ...
  — `runNavigation(scenarios, runner)` iterates all scenarios; calls `navigate` for every scenario; `click` only for scenarios with `clickTarget`; evaluates `location.pathname` and component name; exercises `history.back()` only for `verifyBack=true` scenarios; takes one screenshot per scenario. HTTP error detection added — captures `navigate` return value, checks `httpStatus >= 400`, marks `status='fail'` with `error='HTTP ${httpStatus}'`, takes screenshot, and skips click/evaluate on 5xx.
