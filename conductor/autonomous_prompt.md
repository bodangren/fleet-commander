...
- First track of any calendar day MUST be a `chore` type for refactor/cleanup of previous day's work.
  ...
  Step 4: Verify. Run full test suite (`CI=true bun run test --run`) and production build (`cd app && npm run build`). Fix any new failures.
  Step 5: Finalize.
...
- Commit and push to remote with message: `chore(conductor): Archive <track_id> track; update docs`.
