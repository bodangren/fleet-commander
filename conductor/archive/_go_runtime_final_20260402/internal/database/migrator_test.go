...
func TestMigrateProjectBasic(t *testing.T) {
	...
	os.MkdirAll(filepath.Join(conductorDir, "tracks", "test_track"), 0755)
...
`
	os.WriteFile(filepath.Join(conductorDir, "tracks.md"), []byte(tracksContent), 0644)
...
`
	os.WriteFile(filepath.Join(conductorDir, "tracks", "test_track", "plan.md"), []byte(planContent), 0644)

	// Write an issue
	issueDir := filepath.Join(conductorDir, "broker", "open")
...
func TestMigrateProjectForceSkipsBadProjects(t *testing.T) {
	...
	os.WriteFile(filepath.Join(conductorDir, "tracks.md"), []byte("not valid"), 0644)
