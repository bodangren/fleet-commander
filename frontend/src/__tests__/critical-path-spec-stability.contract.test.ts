...
function hasNamedImport(source: string, symbol: string, moduleSpecifier: string): boolean {
...
describe('critical-path spec stability contract (Phase 3, e2e_test_baseline_hardening_20260619)', () => {
  ...
  describe('per-spec stability patterns', () => {
    it.each(CRITICAL_PATH_SPECS)('$name uses seedScenario (Phase 2 carryover)', name => {
    ...
    it('project.spec.ts seeds a known project state via seedScenario (Phase 3 task 4)', () => {
      ...
      expect(
        /seedScenario\s*\(\s*page\s*,\s*['"](?:demo|empty|kanban-cards)['"]\s*\)/.test(source),
