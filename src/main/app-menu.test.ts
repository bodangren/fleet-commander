...
const getMenuItem = (submenu: any[], label: string) => submenu.find(item => item.label === label)
...
describe('app menu', () => {
  it('builds Open Recent entries from persisted list', () => {
    ...
    expect(openRecent.submenu.map((item: any) => item.label)).toEqual(['/repo/one', '/repo/two'])
  })
