...
| TD-077 | Phase 1 schema.foundation.test.ts expects `for_review`/`med` but existing validators use `review`/`medium` | Test expects task status `for_review` and priority `med`, but existing `taskStatus` validator uses `review` and `priority` uses `medium`. Cannot modify tests per instructions. | Critical: blocks Phase 1 completion |
| TD-078 | New foundation tables conflict with existing schema definitions | Added `projects`, `sprints`, `tasks`, `agents`, `providers`, `pipelineRuns`, `abTests` tables duplicate existing tables in schema.ts (lines 21-32, 34-48, etc.) causing TS1117 "duplicate property" errors. Need to determine if new tables should replace or augment existing ones. | Critical: breaks typecheck |
| TD-079 | Foundation schema changes break 15+ existing Convex/TS files | `sprints.ts`, `retrospectives.ts`, `seedAgents.ts`, `scoreAudit.ts`, `taskRecovery.ts`, `stats.ts`, `projects.ts` route, and others reference old field names (projectSlug, startDate, endDate, goal, taskKeys, etc.) that no longer exist in the new tables. Cannot modify existing files without violating "follow existing patterns" constraint. | Critical: cascade of breakages |
