...
function readStorage(fs: PersistenceFileSystem, userDataPath: string): ProjectStorage | null {
...
export function getLastProjectPath(fs: PersistenceFileSystem, userDataPath: string): string | null {
...
export function getRecentProjects(fs: PersistenceFileSystem, userDataPath: string): string[] {
  const stored = readStorage(fs, userDataPath)
...
export function addRecentProject(
  ...
  const nextRecent = [trimmedPath, ...recent.filter(entry => entry !== trimmedPath)].slice(
    0,
    Math.max(0, limit),
  )
