...
| TD-264 | P0 notification product retirement: remove the public Convex notification module/API, arbitrary webhook, fake email and Pivot routes, orchestrator/budget/retrospective emitters, frontend pages/hooks/routes/nav/settings, and 23 weak wrapper tests. Preserve Alerts, task state/history, and execution logs as operator truth; prove source/API absence and real-Chrome 404/no-navigation/no-API behavior. Do not add per-user checks or mutate the Bounded Factory. | Critical | `notification_authorization_recovery_20260809` |
| TD-265 | Temporary notification schema/data preservation boundary: `notifications` and `notificationPreferences` declarations remain after TD-264 product retirement until an explicit retention, migration, and deletion decision confirms ownership, archival needs, and safe schema/data removal. | High | Separate retention/migration track (not opened) |

> TD-264 is the separately recorded next **P0 security track**, not part of completed TD-263. Bounded Factory acceptance remains approval-gated in its existing track.
