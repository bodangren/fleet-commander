...
- [~] Task: Add a Convex migration/backfill for existing provider rows' new `healthStatus` field (default from last probe or `healthy`). **Red phase covered by `convex/providerHealthVocabulary.test.ts` (`7f6c0b8`)** — 6 assertions on `backfillProviderHealthStatus` (default to healthy, idempotent, preserves operational status, reports backfilledCount). All 6 fail in Red because the export is missing.
