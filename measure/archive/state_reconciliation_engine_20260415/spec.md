...
- **FR1:** `reconciliationRules` config (checked into repo at `measure/reconciliation.yml`) defines per-artifact-class ownership:
...
1. `measure/reconciliation.yml` with rules for task, track metadata, issue, plan.
