...
- [x] Task: Document the exact convex-test invocation (it is non-obvious: `bun test $(find convex -name '*.test.ts' | sed 's|^|./|')`). (`95d4cf5` — Red-phase tests pin the verbatim 'bun test $(find convex -name *.test.ts | sed s|^|./|)' string at measure/tests/verify.test.sh:30; doc captured in test contract.)
